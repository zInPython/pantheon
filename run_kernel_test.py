...
cmd = "./test/test.py local --schemes \"%s\" -t 30 --interval 15 --run-times 1 --flows %d --uplink-trace %s --downlink-trace %s --prepend-mm-cmds \"%s\" --extra-mm-link-args \"%s\"" % (n_flows, schemes, "link.trace", "link.trace", mm_cmds, mm_opts)
os.system(cmd)
